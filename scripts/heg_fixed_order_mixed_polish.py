#!/usr/bin/env python3
"""
HEG fixed-order mixed local-polishing probe.

Start from the promising order-26 graph

    C4=7, C8=1, C16=0, TOTAL=8

and search at fixed order / fixed edge count with two local operator families:

1. two_switch
   remove (a,b),(c,d), add one of the two cross pairings.
   This preserves the complete degree sequence.

2. endpoint_relocate
   orient an old edge as (fixed, donor), require degree(donor) >= 4,
   remove (fixed, donor), and add (fixed, target), where target is a
   non-neighbor of fixed.
   ...
   Therefore this operator can leave the degree-sequence component that trapped
   the pure 2-switch search, while preserving order, edge count and min degree>=3.

Every candidate is independently validated as simple, connected and
minimum-degree >=3, then scored by the existing HEG ScoreWorker.

Search policy:
- default --max-uphill 0 keeps only improving or neutral children;
- neutral moves permit multi-step exploration without an energy increase;
- beam ranking is absolute TOTAL, then weighted score, then C4,C8,C16,...;
- every depth reports exact improving-candidate counts separately for both
  operator families;
- an exact TOTAL=0 graph is saved immediately.

The built-in preset reconstructs and verifies the exact n=26 graph from the
previous strong-rewiring experiment (hash prefix 5ba52272).

Recommended first run:

    uv run python scripts/heg_fixed_order_mixed_polish.py \
      --preset rewire2-n26 \
      --beam-width 32 \
      --max-depth 30 \
      --step-seconds 120 \
      ...
      --witness-cap 1000000 \
      --save-best mixed_polish_best_n26.json \
      --save-lineage mixed_polish_lineage_n26.json
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "Maximum allowed increase in exact TOTAL relative to each parent. "
            "0 allows improving and neutral moves only."
        ),
    )
    parser.add_argument(
        "--step-seconds",
    ...
    if args.max_uphill < 0:
        parser.error("--max-uphill must be >= 0")
    if args.workers < 1:
...
def select_beam(
    ...
    *,
    ...
    exact = [state for state in states if state.score.fully_exact]
    return sorted(
        exact,
...
def main() -> int:
    ...
    console.print(
        f"[bold]HEG fixed-order mixed polish[/bold] "
        f"order={start_graph.order} beam={args.beam_width} "
        f"max_depth={args.max_depth} max_uphill={args.max_uphill} "
        f"workers={args.workers} families={','.join(args.families)}"
    )
    console.print(
    ...
    with ThreadPoolExecutor(
        ...
        for depth in range(1, args.max_depth + 1):
            ...
            console.print(
                ...
                f"improving={improving} neutral={neutral} "
            ...
            for family in args.families:
                ...
                if best_pair is None:
                    ...
                    best_text = (
                        f"{family_best_delta:+d} total={family_best_state.score.total} "
            ...
            if not accepted:
                console.print(
                    f"[yellow]Depth {depth}: no candidate satisfies "
                    f"ΔTOTAL <= {args.max_uphill}. Search is locally trapped.[/yellow]"
                )
                break
            ...
            if not beam:
                console.print(
                    f"[yellow]Depth {depth}: beam became empty.[/yellow]"
                )
                break
