#!/usr/bin/env python3
"""
HEG fixed-order structural/Pareto/escape experiment.

This is the next experiment after the mixed fixed-order search found:

    n=26
    C4=6, C8=1, C16=0
    TOTAL=7

using two mutation families:
    - two_switch
    - endpoint_relocate

The previous search still ranked equal-TOTAL states mainly by the old weighted
score. This experiment instead asks a more structural question:

    "How concentrated are the remaining forbidden cycles?"

For low-TOTAL candidates it enumerates the actual forbidden cycles and computes:

    tau_E               exact minimum edge hitting-set size
    forbidden_edge_union
    max_cycles_per_edge

tau_E is the minimum number of existing graph edges whose union intersects every
currently forbidden cycle. A smaller tau_E means the remaining obstruction is
more concentrated and may be easier to destroy with a local mutation.

Beam layout (defaults for beam=32):
    16 main lanes
       minimum TOTAL first; structural tie-break:
       tau_E, edge-union size, -max edge load, then weighted score

     8 structural/profile lanes
       preserve different (C4,C8,C16,...) profiles and structural signatures,
       so e.g. (7,0,0) is not discarded merely because (6,1,0) has a lower
       historical weighted score

     8 escape lanes
       preserve states up to global_best_TOTAL + escape_height
       (default height 1), allowing paths such as
           7 -> 8 -> 6
           7 -> 8 -> 8 -> 5

Scientific score remains the exact raw forbidden-cycle count. Structural metrics
are search heuristics only; they do not redefine success.

Efficiency:
- all mutation candidates are scored by the existing HEG ScoreWorker;
- structural cycle enumeration runs only on a stratified shortlist
  (--structural-pool, default 512) among states inside the escape barrier;
- because the scorer has already proved exact small counts, the Python witness
  enumerator stops as soon as it has recovered exactly that many canonical
  cycles for a positive component; zero components require no enumeration.

Recommended run from the TOTAL=7 graph saved by the previous experiment:

    uv run python scripts/heg_fixed_order_structural_escape.py \
      ...
      --witness-cap 1000000 \
      --save-best structural_best_n26.json \
      --save-lineage structural_lineage_n26.json
"""
...
class StructuralMetrics:
    ...
    def compact(self) -> str:
        return (
            f"tauE={self.tau_edge} "
            f"unionE={self.forbidden_edge_union} "
            f"maxLoad={self.max_cycles_per_edge}"
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=512,
        help=(
            "Maximum number of exact candidates per depth for which actual "
            "forbidden-cycle structure is enumerated."
        ),
    )
...
def enumerate_exact_cycles(
    ...
    *,
    ...
    """
    Recover the exact canonical simple cycles of one length.

    The external scorer has already proved expected_count exactly. Therefore:
    - expected_count == 0 needs no search;
    - for a positive count we may stop immediately after finding that many
      distinct canonical cycles.

    Canonicalization:
    - the start vertex is the minimum vertex on the cycle;
    - of the two orientations, keep only path[1] < path[-1].
    """
    if expected_count == 0:
        return ()
    if expected_count < 0:
        ...
        def dfs(current: int) -> None:
            ...
            if len(path) == length:
                if start in adjacency[current] and path[1] < path[-1]:
                    cycle_edges = tuple(
                        sorted(
                            [
                                norm_edge(path[index], path[(index + 1) % length])
                                for index in range(length)
                            ]
                        )
                    )
                    found.append(cycle_edges)
            ...
            for nxt in adjacency[current]:
                if stop:
                    return
                if nxt == start:
                    # Do not close a shorter cycle.
                    continue
                if nxt < start:
                    # start must be the minimum vertex in the canonical cycle.
                    continue
                if nxt in used:
                    continue

                # On the final extension, require that the new endpoint can
                # actually close back to start.
                if len(path) == length - 1 and start not in adjacency[nxt]:
                    continue
    ...
    if len(found) != expected_count:
        raise RuntimeError(
            f"cycle witness reconstruction mismatch for C{length}: "
            f"found {len(found)}, scorer proved {expected_count}; "
            f"graph={graph.graph_hash[:8]}"
        )

    if len(set(found)) != len(found):
        raise RuntimeError(
            f"duplicate canonical cycles reconstructed for C{length} "
            f"graph={graph.graph_hash[:8]}"
        )
...
def minimum_edge_hitting_set(
    cycles: tuple[tuple[Edge, ...], ...],
) -> tuple[int, tuple[Edge, ...]]:
    """
    Exact minimum edge hitting set by DP over cycle masks.

    Here TOTAL is deliberately small (typically <=8), so the state space is at
    most 2^TOTAL and the computation is negligible.
    """
    if not cycles:
        return 0, ()
    ...
    target = (1 << len(cycles)) - 1
    infinity = len(cycles) + 1
    distance = [infinity] * (target + 1)
    previous: list[tuple[int, Edge] | None] = [None] * (target + 1)
    distance[0] = 0

    for mask in range(target + 1):
        if distance[mask] == infinity:
            continue
        for edge, edge_mask in edge_masks.items():
            nxt = mask | edge_mask
            candidate = distance[mask] + 1
            if candidate < distance[nxt]:
                distance[nxt] = candidate
                previous[nxt] = (mask, edge)

    if distance[target] == infinity:
        raise RuntimeError("failed to construct edge hitting set")
...
def compute_structural_metrics(state: BeamState) -> StructuralMetrics:
    score = state.score
    ...
    for component in sorted(score.components, key=lambda item: item.length):
        if component.observed == 0:
            continue
        cycles = enumerate_exact_cycles(
            score.graph,
            length=component.length,
            expected_count=component.observed,
        )
        all_cycles.extend(cycles)

    if len(all_cycles) != score.total:
        raise RuntimeError(
            f"structural cycle total mismatch: witnesses={len(all_cycles)} "
            f"score={score.total} graph={score.graph.graph_hash[:8]}"
        )
    ...
    tau_edge, hitting_edges = minimum_edge_hitting_set(tuple(all_cycles))

    return StructuralMetrics(
        tau_edge=tau_edge,
        forbidden_edge_union=len(loads),
        max_cycles_per_edge=max(loads.values()),
        hitting_edges=hitting_edges,
...
def stratified_structural_shortlist(
    ...
    *,
    ...
    """
    Deterministic profile/family/parent-stratified shortlist for the more
    expensive structural witness analysis.

    Lower TOTAL is always processed first. Within one TOTAL, candidates are
    round-robined across component profiles and mutation families, with graph
    hash providing deterministic sampling rather than the old weighted score.
    """
    barrier = global_best_total + escape_height
    eligible = [
        state
        for state in states
        if state.score.fully_exact
        and state.score.total is not None
        and state.score.total <= barrier
    ]

    by_total: dict[int, list[BeamState]] = defaultdict(list)
    for state in eligible:
        assert state.score.total is not None
        by_total[state.score.total].append(state)
        ...
        while keys and len(selected) < limit:
            ...
            for key in keys:
                ...
                if index < len(bucket):
                    ...
                    if graph_hash not in selected_hashes:
...
def main() -> int:
    ...
    with ThreadPoolExecutor(
        ...
        root_metrics = compute_structural_metrics(root)
        ...
        for depth in range(1, args.max_depth + 1):
            ...
            if zero_hits:
                ...
                hit_metrics = compute_structural_metrics(hit_state)
            ...
            shortlist = stratified_structural_shortlist(
                exact_pool,
            ...
            if missing:
                futures = {
                    executor.submit(compute_structural_metrics, state): state
            ...
            if best.score.graph.graph_hash not in structural_cache:
                structural_cache[best.score.graph.graph_hash] = (
                    compute_structural_metrics(best)
                )
