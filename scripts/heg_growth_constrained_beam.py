#!/usr/bin/env python3
"""
HEG constrained-growth beam probe.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="c16",
        help=(
            "Objective inside the hard exact ΔC4=0 feasible set. c16 minimises "
            "absolute C16 first, then ΔC16, C8 and total. total minimises the "
            "absolute total forbidden-cycle count first."
        ),
    )
    parser.add_argument(
        "--step-seconds",
...
def exact_zero_c4_delta(state: BeamState) -> bool:
    """Return True only when the true ΔC4 is proved exactly zero."""
    lower, upper = delta_bounds(state, 4)
    return upper is not None and lower == 0 and upper == 0


def constrained_rank_key(state: BeamState, secondary: str) -> tuple[int, ...]:
    """
    Ranking inside the hard ΔC4=0 feasible set.

    c16:
      absolute C16 -> ΔC16 -> absolute C8 -> ΔC8 -> ΔSUM -> total.
    total:
      absolute total -> ΔSUM -> C16 -> C8.

    Absolute C16 is cumulative C16 growth from the common root because the
    experiment only adds structure and all lineages start from the same graph.
    """
    if not exact_zero_c4_delta(state):
        return (1,)

    c16_abs = component_true_count_key(state, 16)
    c16_delta = component_delta_key(state, 16)
    c8_abs = component_true_count_key(state, 8)
    c8_delta = component_delta_key(state, 8)
...
def select_constrained_beam(
    states: list[BeamState],
    *,
    beam_width: int,
    secondary: str,
) -> list[BeamState]:
    feasible = [state for state in states if exact_zero_c4_delta(state)]
    return sorted(
        feasible,
        key=lambda state: (
            constrained_rank_key(state, secondary),
            state.score.graph.graph_hash,
        ),
    )[:beam_width]


def print_final_constrained_lineage(final_state: BeamState, secondary: str) -> None:
    path = lineage_path(final_state)
    final_order = final_state.score.graph.order
    lengths = tuple(int(length) for length in forbidden_lengths(final_order))
    ...
    console.print()
    console.print(
        f"[bold]FINAL CONSTRAINED LINEAGE[/bold] constraint=ΔC4=0 "
        f"secondary={secondary} steps={len(path)-1} "
        f"order={path[0].score.graph.order}->{final_order}"
    )
    cycle_headers = " ".join(
        f"{('ΔC'+str(length)+' [C]'):>19}" for length in lengths
    )
    console.print(
        f"{'N':>3} {cycle_headers} {'ΔSUM':>12} {'ATTACH':>15} {'HASH':>8} STATE"
    )
    ...
    for index, state in enumerate(path):
        ...
        for length in lengths:
            ...
            value = (
                f"- [{absolute}]"
                if index == 0
                else f"{delta_text_for_report(state, length)} [{absolute}]"
            )
            cells.append(f"{value:>19}")
        delta_sum = "-" if index == 0 else total_delta_text_for_report(state)
        attachment = "-" if state.attachment is None else str(state.attachment)
        console.print(
            f"{state.score.graph.order:>3} " + " ".join(cells)
            + f" {delta_sum:>12} {attachment:>15} "
            f"{state.score.graph.graph_hash[:8]:>8} {score_status(state.score)}"
        )
    ...
    zero_prefix = 0
    for state in path[1:]:
        if exact_zero_c4_delta(state):
            zero_prefix += 1
        else:
            break
    root_c4 = path[0].score.component_map()[4].observed
    final_c4 = final_state.score.component_map()[4].observed
    console.print(
        f"Constraint summary: exact ΔC4=0 prefix={zero_prefix}/{len(path)-1}, "
        f"C4 {root_c4}->{final_c4}"
    )


def save_constrained_lineage(path: Path, final_state: BeamState, secondary: str) -> None:
    states = lineage_path(final_state)
    payload = {
        "schema_version": "graphoratory.heg_constrained_growth_lineage.v1",
        "constraint": "exact_delta_c4_zero",
        "secondary": secondary,
        "root_order": states[0].score.graph.order,
        "final_order": states[-1].score.graph.order,
        "states": [],
    ...
    for index, state in enumerate(states):
        ...
        payload["states"].append({
            ...
            "constraint_satisfied": None if index == 0 else exact_zero_c4_delta(state),
            "components": {
                str(length): {
                    "observed": component.observed,
                    "lower": component.lower,
                    "upper": component.upper,
                    "status": component.status,
                    "delta_lower": None if index == 0 else delta_bounds(state, length)[0],
                    "delta_upper": None if index == 0 else delta_bounds(state, length)[1],
                }
                for length, component in sorted(components.items())
            },
        })
    ...
    def print_step(
        ...
        *,
        ...
        if pool and scored is not None and space is not None:
            ...
            note = (
                "minima within feasible ΔC4=0 candidates"
                if exhaustive
                else "* minima within feasible ΔC4=0 scored candidates"
            )
...
def main() -> int:
    ...
    console.print(
        f"[bold]HEG constrained-growth beam probe[/bold] "
        f"start={args.start_order} target={args.target_order} "
        f"beam={args.beam_width} workers={args.workers} constraint=ΔC4=0 secondary={args.secondary}"
    )
    console.print(
    ...
    console.print(
        "[dim]Only children with provably exact ΔC4=0 survive. Among them the "
        "beam optimises the secondary objective and preserves one coherent ancestry.[/dim]"
    )
    ...
    with ThreadPoolExecutor(
        ...
        for step in range(1, args.target_order - args.start_order + 1):
            ...
            feasible_pool = [state for state in pool if exact_zero_c4_delta(state)]
            beam = select_constrained_beam(
                pool,
            ...
            if not beam:
                console.print(
                    f"[yellow]Step {step}: no scored child proved exact ΔC4=0; "
                    "constrained search terminates.[/yellow]"
                )
                console.print(f"     feasible ΔC4=0: 0/{len(pool)} scored")
                break

            leader = min(
                beam,
                key=lambda state: (
                    constrained_rank_key(state, args.secondary),
                    state.score.graph.graph_hash,
                ),
            )
            ...
            console.print(
                f"     feasible ΔC4=0: {len(feasible_pool)}/{len(pool)} scored"
            )
    ...
    # Pick the final leader of the constrained beam.
    final_leader = min(
        beam,
        key=lambda state: (
            constrained_rank_key(state, args.secondary),
    ...
    if args.save_lineage is not None:
        save_constrained_lineage(args.save_lineage, final_leader, args.secondary)
        console.print(f"Final lineage saved: {args.save_lineage}")

    print_final_constrained_lineage(final_leader, args.secondary)
    ...
    console.print(
        f"[bold]Done[/bold] constraint=ΔC4=0 secondary={args.secondary} order={final_leader.score.graph.order} "
