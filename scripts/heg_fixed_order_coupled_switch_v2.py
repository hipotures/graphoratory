#!/usr/bin/env python3
"""
HEG fixed-order coupled-switch v2 with exact-C4 admissible pruning.

Why v2 exists
-------------
The first coupled_switch2 run evaluated ~32k coupled candidates per depth and
found no neutral/improving move. The best sampled coupled moves were usually
TOTAL 11-22 from an incumbent TOTAL 7.

That run exposed a selection defect: the generator ranked proposals primarily
by how many existing forbidden cycles their REMOVED edges hit, but ignored how
many new C4s the ADDED edges create.

v2 fixes that without weakening scientific correctness.

Exact C4 prefilter
------------------
For every structurally legal candidate, before the external HEG scorer runs, v2
computes the exact number of C4 cycles using

    C4 = 1/2 * sum_{u<v} binom(|N(u) ∩ N(v)|, 2).

At n=26 this is only 325 vertex pairs and bit_count operations.

Because the objective is

    TOTAL = C4 + C8 + C16 + ...

with every component non-negative, a candidate with

    exact_C4 > current_global_best_TOTAL + escape_height

cannot possibly enter the beam.

Such a candidate is therefore rejected BEFORE the expensive sglab-score-worker
call. This is an exact admissible pruning rule, not a heuristic.

The scorer remains authoritative for every candidate that passes the C4 gate.
For fully exact C4 scorer output, v2 also cross-checks the scorer's C4 against
the fast formula and fails loudly on mismatch.

Wider coupled search
--------------------
The coupled generator now has an independent scan budget:

    --coupled-scan-per-parent

Default: 8192 candidates per beam parent.

Unlike v1, it does not consume the cap exclusively from the highest
removed-cycle-coverage plans. It interleaves:

    - coverage-first plans;
    - deterministic seeded exploration across the whole legal plan set.

This allows the exact-C4 gate, rather than the removal-only heuristic, to decide
which coupled rewrites deserve expensive scoring.

Scoring pipeline
----------------
The optimized multiprocessing architecture is retained:

    parent process:
        generate mutation descriptors

    score processes:
        apply mutation
        fast fixed-order legality/connectivity validation
        exact C4
        admissible prune
        graph hash
        full HEG score only for survivors

Each score process owns one persistent sglab-score-worker subprocess.

Recommended first run
---------------------
Use direct/neutral search first:

    uv run python scripts/heg_fixed_order_coupled_switch_v2.py \
      --start-graph coupled_best_n26.json \
      --beam-width 32 \
      --main-lanes 24 \
      --structural-lanes 8 \
      --escape-lanes 0 \
      ...
      --witness-cap 1000000 \
      --save-best coupled_v2_best_n26.json \
      --save-lineage coupled_v2_lineage_n26.json
"""
...
def main() -> int:
    ...
    console.print(
        f"[bold]HEG fixed-order coupled switch v2 + exact C4 gate[/bold] "
