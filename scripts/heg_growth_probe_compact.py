...
def _component_cell(
    result: GraphScore,
    length: int,
    previous: GraphScore | None,
) -> str:
    component = result.component_map().get(length)
    if component is None:
        return "-"

    if component.status == STATUS_EXACT:
        value = str(component.observed)
    elif component.status == STATUS_SATURATED:
        value = f">={component.observed}"
    else:
        value = f"[{component.lower},{component.upper}]"

    if previous is None:
        return value

    old = previous.component_map().get(length)
    if component.status == STATUS_EXACT and old is not None and old.status == STATUS_EXACT:
        return f"{value} ({component.observed - old.observed:+d})"

    if component.status == STATUS_EXACT and old is None:
        return f"{value} (+{component.observed})"

    return f"{value} (?)"


def _short_state(result: GraphScore, *, committed: bool) -> str:
    if result.fully_exact:
        return "OK" if committed else "STOP"

    sat = [str(c.length) for c in result.components if c.status == STATUS_SATURATED]
    budget = [str(c.length) for c in result.components if c.status == STATUS_BUDGET]

    parts: list[str] = []
    if sat:
        parts.append("CAP C" + ",C".join(sat))
    if budget:
        parts.append("BUD C" + ",C".join(budget))
    prefix = "" if committed else "STOP "
    return prefix + " ".join(parts)


class StepLedger:
    """Print one compact header once, then one compact row per growth step."""

    def __init__(self, cycle_lengths: tuple[int, ...]) -> None:
        self.cycle_lengths = cycle_lengths
        self._header_printed = False

    def _table(self, *, header: bool) -> Table:
        table = Table(
            show_header=header,
            show_edge=False,
            show_lines=False,
        ...
        table.add_column("STEP", justify="right", width=4, no_wrap=True)
        table.add_column("N", justify="right", width=3, no_wrap=True)
        table.add_column("TRIED", justify="right", width=11, no_wrap=True)
        for length in self.cycle_lengths:
            table.add_column(f"C{length}", justify="right", min_width=7, no_wrap=True)
        table.add_column("ATTACH", min_width=10, no_wrap=True)
    ...
    def print_row(
        ...
        *,
        ...
        table = self._table(header=not self._header_printed)
        self._header_printed = True

        tried_text = "-" if tried is None or space is None else f"{tried}/{space}"
        attach_text = "-" if result.attachment is None else str(result.attachment)

        cells = [
            str(step),
            str(result.graph.order),
            tried_text,
            *[
                _component_cell(result, length, previous)
                for length in self.cycle_lengths
            ],
            attach_text,
            f"{elapsed:.2f}s" if step else f"{elapsed:.3f}s",
            _short_state(result, committed=committed),
        ]
        table.add_row(*cells)
        console.print(table)
...
def main() -> int:
    ...
    with ThreadPoolExecutor(
        ...
        ledger = StepLedger(
            tuple(int(length) for length in forbidden_lengths(args.target_order))
        )
        ledger.print_row(
            step=0,
            ...
            tried=None,
            space=None,
            elapsed=current_score.elapsed_seconds,
        )
