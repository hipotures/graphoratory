#!/usr/bin/env python3
"""
HEG n=24 random legal-walk experiment.

Purpose
-------
Explore the full fixed-order admissible state space

    simple, connected, undirected, |V| = 24, delta(G) >= 3

without using C4/C8/C16 witnesses, hitting sets, hot vertices, structural
rankings, or any other cycle-derived information to construct mutations.

Every scoring process does both jobs locally:

    parent -> random legal edge walk -> HEG evaluation

The parent process only chooses parents, dispatches deterministic work batches,
collects scores, maintains a score-blind random reservoir, records an optional
elite set, and writes artifacts. Candidate generation is never performed in the
parent process.

Random mutation kernel
----------------------
A mutation is a random walk in the space of legal graphs. Each micro-step is
chosen randomly as ADD or REMOVE:

* ADD chooses a uniformly random current non-edge and inserts it.
* REMOVE chooses a random edge whose deletion preserves min degree >= 3 and
  connectivity.

All intermediate states remain legal. This avoids the extremely low acceptance
rate of "flip arbitrary adjacency bits and reject at the end", while remaining
cycle-blind. Starting from a cubic graph is not a restriction: ADD creates
slack, and later REMOVE steps can change the degree sequence and topology.
The walk may end at any edge count >= 3n/2; --max-edges=0 means no artificial
upper edge-count cap.

By default parent selection is also score-blind:

    --elite-parent-prob 0.0

so the evaluated HEG score does not influence where the random walk goes.
Set a positive elite-parent probability later if you want a hybrid random-search
mode. In either mode the mutation itself remains completely cycle-blind.

Scientific scoring
------------------
The external sglab/HEG scorer is authoritative. A candidate contributes to the
best result only when every forbidden-cycle component is exact. Saturated or
budget-exhausted results are counted and reported but never treated as exact.

Recommended first run
---------------------

uv run python scripts/heg_random_legal_walk.py \
  --start-graph markstroem_slack_best_n24.json \
  --expected-order 24 \
  --workers 16 \
  --candidates-per-worker 8 \
  --walk-min 4 \
  --walk-max 48 \
  --elite-parent-prob 0.0 \
  ...
  --witness-cap 1000000 \
  --save-best random_walk_best_n24.json \
  --save-hits random_walk_hits_n24.jsonl \
  --save-pool random_walk_pool_n24.json
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.05,
        help="Probability of restarting a walk from the original start graph.",
    )
    ...
    with ProcessPoolExecutor(
        ...
        while time.perf_counter() < deadline:
            ...
            for _worker_slot in range(args.workers):
                ...
                for _ in range(args.candidates_per_worker):
                    parent = choose_parent(
                        ...
                        elite_probability=args.elite_parent_prob,
                    )
            ...
            if now >= next_report or found_success:
                ...
                console.print(
                    f"STATUS t={elapsed:.1f}s rounds={rounds} evaluated={evaluated:,} "
