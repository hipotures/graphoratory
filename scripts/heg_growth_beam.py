...
Default selection mode is "portfolio": the next beam mixes candidates that are
best for total forbidden-cycle count, weighted count, and each individual
forbidden length. This is deliberately less myopic than pure lexicographic C4,
C8, C16, ... selection.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--objective",
        choices=("portfolio", "total", "weighted", "lex"),
        default="portfolio",
        help=(
            "portfolio keeps specialists for total, weighted, C4, C8, ...; "
            "total minimises total forbidden cycles; weighted emphasises short "
            "cycles; lex minimises C4 then C8 then C16, ..."
        ),
    )
    parser.add_argument(
        "--step-seconds",
...
def rank_key(state: BeamState, objective: str) -> tuple[int, ...]:
    score = state.score
    upper_vector = tuple(c.upper for c in score.components)
    lower_vector = tuple(c.lower for c in score.components)

    if objective == "lex":
        return (
            *upper_vector,
            score.non_exact_components,
            *lower_vector,
            score.upper_total,
            score.upper_weighted,
        )
    if objective == "weighted":
        return (
            ...
            score.non_exact_components,
            *upper_vector,
            score.lower_weighted,
        )

    # "total" and the final tie-break for "portfolio".
    return (
        score.upper_total,
        score.upper_weighted,
        score.non_exact_components,
...
def _ranking_for_component(
    ...
    return sorted(
        ...
        key=lambda state: (
            ...
            state.score.non_exact_components,
...
def select_beam(
    ...
    *,
    ...
    if len(states) <= beam_width:
        return sorted(states, key=lambda s: (rank_key(s, objective), s.score.graph.graph_hash))
    ...
    lengths = tuple(c.length for c in states[0].score.components)
    ...
    rankings: list[list[BeamState]] = [
        sorted(
            ...
            key=lambda s: (
                s.score.upper_total,
                s.score.upper_weighted,
                s.score.non_exact_components,
                s.score.graph.graph_hash,
            ),
        ),
        sorted(
            states,
    ...
    # Round-robin over objective-specific rankings. This keeps multiple structural
    # directions alive instead of letting one scalar/lexicographic objective
    # collapse the search immediately.
    ...
    if len(selected) < beam_width:
        for candidate in sorted(states, key=lambda s: (rank_key(s, "total"), s.score.graph.graph_hash)):
            graph_hash = candidate.score.graph.graph_hash
            if graph_hash in selected_hashes:
                continue
            selected.append(candidate)
...
class Ledger:
    def __init__(self, target_order: int) -> None:
        self.lengths = tuple(int(length) for length in forbidden_lengths(target_order))
    ...
    def print_row(
        ...
        *,
        ...
        if not self._printed_header:
            cycle_headers = " ".join(f"{('C'+str(length)):>17}" for length in self.lengths)
            console.print(
                f"{'STEP':>4} {'N':>3} {'TRIED':>9} {'UNIQ':>9} {'BEAM':>4} "
                f"{cycle_headers} {'TIME':>8}  STATE"
            )
            self._printed_header = True

        current = leader.score.component_map()
        parent = leader.parent_score.component_map() if leader.parent_score is not None else {}

        cycle_cells: list[str] = []
        for length in self.lengths:
            component = current.get(length)
            value = compact_value(component)
            old = parent.get(length)
            if (
                component is not None
                and component.status == STATUS_EXACT
                and old is not None
                and old.status == STATUS_EXACT
            ):
                value = f"{value}({component.observed - old.observed:+d})"
            elif (
                component is not None
                and component.status == STATUS_EXACT
                and old is None
            ):
                value = f"{value}(+{component.observed})"
            cycle_cells.append(f"{value:>17}")

        tried_text = "-" if tried is None else str(tried)
        unique_text = "-" if unique is None else str(unique)
        console.print(
            f"{step:>4} {leader.score.graph.order:>3} {tried_text:>9} "
            f"{unique_text:>9} {beam_size:>4} "
            + " ".join(cycle_cells)
...
def main() -> int:
    ...
    console.print(
        f"[bold]HEG beam-growth probe[/bold] "
    ...
    console.print(
        "[dim]Pure vertex addition cannot destroy old cycles. Beam search only "
        "prevents one greedy path from prematurely collapsing the search.[/dim]"
    )
    ...
    ledger = Ledger(args.target_order)
        ...
        for step in range(1, args.target_order - args.start_order + 1):
            ...
            leader = min(
                ...
                key=lambda state: (
                    rank_key(
                        state,
                        "total" if args.objective == "portfolio" else args.objective,
                    ),
                    state.score.graph.graph_hash,
                ),
            )
    ...
    console.print(
        ...
        f"beam={len(beam)} total_upper={final_leader.score.upper_total} "
