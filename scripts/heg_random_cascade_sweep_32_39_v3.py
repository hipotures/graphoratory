...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument('--root-node-budget', type=int, default=250_000_000)
    p.add_argument('--fresh-seed-retries', type=int, default=12)
    ...
    p.add_argument(
        '--child-script',
        type=Path,
        default=Path('scripts/heg_random_cascade_budget_v3.py'),
    )
    p.add_argument(
        '--output-dir',
        type=Path,
    ...
    if args.fresh_seed_retries < 0:
        p.error('--fresh-seed-retries must be >= 0')
...
def write_fresh_seed(
    *,
    ...
    edges = random_minimal_legal_seed(order, seed_value)
    path = out / f'fresh_seed_attempt_{attempt:02d}.json'
    payload = {
        'schema_version': 'heg.random_minimum_edge_seed.v1',
        'order': order,
        'edges': [list(edge) for edge in edges],
...
def run_child(
    *,
    ...
    command = [
        ...
        '--root-node-budget', str(args.root_node_budget),
...
def main() -> int:
    ...
    for order in args.orders:
        ...
        while True:
            run_log = out / (
                'run.log' if attempt == 0 else f'bootstrap_attempt_{attempt:02d}.log'
            )
            print(
                f'\n=== n={order} source={source} attempt={attempt} start={start} ===',
                flush=True,
            )
            rc = run_child(
                args=args,
            ...
            if rc == 0:
                # Keep the successful log under the conventional name.
                if run_log.name != 'run.log':
            ...
            root_incomplete = (
                'start graph score is not exact at C' in text
                or 'start graph score hit witness cap at C' in text
            )

            # Existing/explicit starts should already be exact.  Do not silently
            # replace them.  Automatic retries are only for generated fresh roots.
            if not source.startswith('fresh') or not root_incomplete:
                print(
                    f'n={order} failed with exit code {rc}; see {run_log}',
                    flush=True,
                )
                return rc

            if attempt >= args.fresh_seed_retries:
                print(
                    f'n={order}: exhausted {args.fresh_seed_retries} fresh-root retries; '
                    f'last log: {run_log}',
                    flush=True,
                )
                return rc
            ...
            attempt += 1
            seed_value = args.seed + order * 1_000_003 + attempt * 104_729
            start = write_fresh_seed(
                order=order,
                out=out,
                seed_value=seed_value,
                attempt=attempt,
            )
            source = 'fresh-minimum-edge-retry'
            print(
                f'n={order}: bootstrap was not exact; retrying with fresh '
                f'minimum-edge seed #{attempt}',
                flush=True,
            )
        ...
        if best_path.exists():
            ...
            if isinstance(score, dict):
                row['best_total'] = score.get('total')
                row['best_weighted'] = score.get('weighted')
                row['best_components'] = score.get('components')
            row['edge_count'] = len(payload.get('edges', []))
            row['graph_hash'] = payload.get('graph_hash')
        if summary_path.exists():
            meta = json.loads(summary_path.read_text(encoding='utf-8'))
            row['evaluated'] = meta.get('evaluated')
            row['exact'] = meta.get('exact')
            row['certified_pruned'] = meta.get('certified_pruned')
            row['elapsed_seconds'] = meta.get('elapsed_seconds')
        sweep_summary.append(row)

        print(
            f"n={order} DONE T={row.get('best_total')} "
            f"eval={row.get('evaluated')} retries={attempt}",
            flush=True,
        )

    summary_file = args.output_dir / 'sweep_summary.json'
    summary_file.write_text(
        json.dumps(sweep_summary, indent=2, sort_keys=True) + '\n',
        encoding='utf-8',
    )
    print(f'\nSweep summary: {summary_file}', flush=True)
    return 0
