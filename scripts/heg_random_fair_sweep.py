#!/usr/bin/env python3
"""
Fair blind HEG random-walk sweep from independently randomized fresh seeds.

Runs the existing cycle-blind worker-local mutator independently for each order.
Default target set:

    n = 23, 24, ..., 33
    450 seconds per order
...
Start graph policy
------------------
Every order starts from an independently randomized minimum-edge legal graph,
generated without looking at C4/C8/C16/C32 or any score.  For even n the start
is a random cubic graph.  For odd n the start has one degree-4 vertex and all
other degrees 3.  Thus all orders receive the same kind of neutral fresh start.

The generator uses a configuration-model pairing with rejection of loops,
parallel edges, and disconnected outcomes.  --seed makes the whole experiment
reproducible while still giving different start topologies across orders.
Explicit --start ORDER=PATH remains available for a deliberate override.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("random_fair_sweep_23_33"),
    )
    parser.add_argument(
        "--seed-dir",
        action="append",
...
def write_summary(path: Path, summaries: list[OrderSummary], args: argparse.Namespace) -> None:
    payload = {
        "schema_version": "heg.random_fair_sweep.v1",
