...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default=-1,
        help=(
            "Enable a fixed-C4 control sample. For this experiment use 4. "
            "Candidates with exactly this many C4 cycles are scored with a "
            "comparison threshold at least this large so higher forbidden "
            "cycles can be certified even if the search incumbent improves."
        ),
    )
    p.add_argument(
        "--control-reservoir-size-per-phase",
        type=int,
        default=2500,
        help=(
            "Reservoir size for certified C4-control graphs in EACH phase "
            "(RANDOM and ELITE). Memory remains bounded."
        ),
    )
    p.add_argument(
        "--save-control",
        type=Path,
        default=Path("c4_control_higher_present.jsonl"),
        help=(
            "Bounded reservoir sample of graphs with C4=--control-c4 and a "
            "certified positive higher forbidden-cycle component."
        ),
    )
    ...
    if args.control_c4 < -1:
        p.error("--control-c4 must be -1 (disabled) or >= 0")
    if args.control_reservoir_size_per_phase < 1:
        p.error("--control-reservoir-size-per-phase must be >= 1")
...
def control_candidate_proof(
    result: CandidateResult,
    target_c4: int,
) -> ComponentResult | None:
    """Return the later component certifying 'higher forbidden cycle present'.

    Preconditions for a valid control candidate:
      * C4 was counted exactly and equals target_c4;
      * the cascade was dispatched with threshold >= target_c4;
      * pruning occurred only at a later forbidden length.

    In the dedicated n=32 experiment the start graph must already have TOTAL <= 4,
    so the forced threshold is exactly 4. Then any later prune proves that the
    graph has at least one C8/C16/C32 while keeping C4 fixed at four.
    """
    if target_c4 < 0 or not result.components:
        return None
    first = result.components[0]
    if (
        first.length != 4
        or first.status != STATUS_EXACT
        or first.observed != target_c4
    ):
        return None
    if result.score_status != STATUS_PRUNED:
        return None
    later = [component for component in result.components if component.length > 4]
    if not later:
        return None
    return later[-1]


def update_control_reservoir(
    reservoir: list[ControlEntry],
    reservoir_hashes: set[str],
    entry: ControlEntry,
    *,
    capacity: int,
    ...
    """Bounded reservoir sample; hashes are retained only for current slots."""
    if entry.graph_hash in reservoir_hashes:
        return False
    if len(reservoir) < capacity:
        reservoir.append(entry)
...
def save_control_sample(
    path: Path,
    *,
    samples_by_phase: dict[str, list[ControlEntry]],
    target_c4: int,
) -> None:
    """Atomically rewrite the current bounded control reservoir."""
    if target_c4 < 0:
        return
    path.parent.mkdir(parents=True, exist_ok=True)
    tmp = path.with_name(path.name + ".tmp")
    with tmp.open("w", encoding="utf-8") as fh:
        for phase in ("RANDOM", "ELITE"):
            for entry in samples_by_phase.get(phase, []):
                payload = {
                    "schema_version": "heg.c4_control_candidate.v1",
                    "control_kind": "fixed_c4_higher_forbidden_present",
                    "target_c4": target_c4,
                    "phase": entry.phase,
                    ...
                    "components_prefix": {
                        str(length): {"observed": observed, "status": status}
                        for length, observed, status in entry.components
                    },
...
def main() -> int:
    ...
    if args.control_c4 >= 0 and root.total > args.control_c4:
        raise RuntimeError(
            f"C4-control collection requires a start graph with exact TOTAL <= "
            f"{args.control_c4}; root TOTAL={root.total}. "
            "For n=32 start from the existing TOTAL=4 best graph."
        )
    ...
    if args.control_c4 >= 0 and args.save_control.exists():
        args.save_control.unlink()
    ...
    metadata: dict[str, object] = {
        ...
        "control_c4": args.control_c4,
        "control_reservoir_size_per_phase": args.control_reservoir_size_per_phase,
        "control_policy": (
            "stratified RANDOM/ELITE reservoir of exact C4=k candidates with "
            "certified positive higher forbidden-cycle component"
            if args.control_c4 >= 0
            else "disabled"
        ),
    ...
    control_samples: dict[str, list[ControlEntry]] = {"RANDOM": [], "ELITE": []}
    control_hashes: dict[str, set[str]] = {"RANDOM": set(), "ELITE": set()}
    control_seen: Counter[str] = Counter()
    control_insertions: Counter[str] = Counter()
    control_clean_exact: Counter[str] = Counter()
    control_unknown: Counter[str] = Counter()
    ...
    with ProcessPoolExecutor(
        ...
        while evaluated < args.evaluation_budget:
            ...
            for _ in range(target_requests):
                ...
                requests_all.append(
                    CandidateRequest(
                        ...
                        incumbent_total=(
                            max(best.total, args.control_c4)
                            if args.control_c4 >= 0
                            else best.total
                        ),
                    )
            ...
            for result in round_results:
                ...
                if args.control_c4 >= 0 and result.components:
                    first = result.components[0]
                    if (
                        first.length == 4
                        and first.status == STATUS_EXACT
                        and first.observed == args.control_c4
                    ):
                        proof = control_candidate_proof(result, args.control_c4)
                        if proof is not None:
                            control_seen[phase_name] += 1
                            control_entry = ControlEntry(
                                ...
                                components=tuple(
                                    (component.length, component.observed, component.status)
                                    for component in result.components
                                ),
                            )
                            if update_control_reservoir(
                                control_samples[phase_name],
                                control_hashes[phase_name],
                                control_entry,
                                capacity=args.control_reservoir_size_per_phase,
                                stream_seen=control_seen[phase_name],
                                rng=rng,
                            ):
                                control_insertions[phase_name] += 1
                        elif result.fully_exact:
                            # With threshold >= target C4, exact TOTAL==target C4
                            # means all higher forbidden components are zero.
                            if result.total == args.control_c4:
                                control_clean_exact[phase_name] += 1
                        elif result.score_status in {STATUS_BUDGET, STATUS_SATURATED}:
                            control_unknown[phase_name] += 1
            ...
            if (
                ...
                if args.control_c4 >= 0:
                    console.print(
                        f"       C4-control k={args.control_c4} "
                        f"dirty_seen[R/E]={control_seen['RANDOM']:,}/{control_seen['ELITE']:,} "
                        f"sample[R/E]={len(control_samples['RANDOM']):,}/{len(control_samples['ELITE']):,} "
                        f"clean_exact[R/E]={control_clean_exact['RANDOM']:,}/{control_clean_exact['ELITE']:,} "
                        f"unknown[R/E]={control_unknown['RANDOM']:,}/{control_unknown['ELITE']:,}"
                    )
                    save_control_sample(
                        args.save_control,
                        samples_by_phase=control_samples,
                        target_c4=args.control_c4,
                    )
    ...
    metadata.update(
        {
            ...
            "control_dirty_seen_by_phase": dict(control_seen),
            "control_insertions_by_phase": dict(control_insertions),
            "control_clean_exact_by_phase": dict(control_clean_exact),
            "control_unknown_by_phase": dict(control_unknown),
            "control_sample_size_by_phase": {
                phase: len(items) for phase, items in control_samples.items()
            },
        }
    )
    ...
    if args.control_c4 >= 0:
        console.print(f"C4 control sample saved: {args.save_control}")
    return 0
