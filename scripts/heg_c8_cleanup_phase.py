...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Two-phase HEG cleanup: acquire C4<=target, then minimize remaining forbidden cycles under a hard C4 cap."
        )
    )
    ...
    parser.add_argument(
        ...
        default=parse_families(
            "slack_edge_add,c8_two_switch,c8_endpoint_relocate,"
            "endpoint_relocate,two_switch,coupled_switch2,"
            "vertex_rewire2,excess_edge_remove"
        ),
        help=(
            "Families used after a state with C4<=--target-c4-cap is found. "
            "The cleanup phase enforces the C4 cap exactly."
        ),
    )
    parser.add_argument(
        "--target-c4-cap",
        type=int,
        default=2,
        help="Enter cleanup phase once C4 is at most this value.",
    )
    parser.add_argument(
        "--cleanup-escape-height",
        type=int,
        default=2,
        help=(
            "In cleanup phase keep states with TOTAL <= best cleanup TOTAL + "
            "this value, while enforcing C4<=target cap."
        ),
    )
    parser.add_argument(
        "--success-total",
        ...
        default=2,
        help=(
            "Stop immediately once an exact state with TOTAL at most this "
            "value is found."
        ),
    )
    parser.add_argument(
        ...
        default=2048,
        help=(
            "Maximum proposals from each C8-targeted family for each beam parent."
        ),
    )
    parser.add_argument(
        "--save-frontier",
        ...
        default=None,
        help=(
            "Optional JSON path for the best C4-constrained cleanup frontier "
            "even when --success-total is not reached."
        ),
    )
    parser.add_argument(
    ...
    if args.target_c4_cap < 0:
        parser.error("--target-c4-cap must be >= 0")
    if args.cleanup_escape_height < 0:
        parser.error("--cleanup-escape-height must be >= 0")
    if args.success_total < 0:
        parser.error("--success-total must be >= 0")
    if args.success_total > args.target_c4_cap:
        parser.error("--success-total must be <= --target-c4-cap")
    if args.c8_limit_per_parent < 0:
        parser.error("--c8-limit-per-parent must be >= 0")
    if args.max_depth < 0:
...
def slack_edge_add_mutations(
    graph: Graph,
    *,
    max_edge_excess: int,
) -> list[Mutation]:
    """
    Add one non-edge without changing order.

    This is deliberately simpler than slack_rewire*: after C4 has already been
    reduced to the cleanup cap, a pure +1 edge can create degree slack cheaply.
    The exact C4 gate decides whether the added edge is admissible.
    """
    if graph_edge_excess(graph) >= max_edge_excess:
        return []

    edge_set = set(graph.edges)
    return [
        Mutation(
            family="slack_edge_add",
            removed_edges=(),
            added_edges=(norm_edge(u, v),),
            variant=f"u={u}/v={v}",
        )
...
def _component_observed(score: GraphScore, length: int) -> int:
    component = score.component_map().get(length)
    return 0 if component is None else component.observed


def c8_two_switch_mutations(
    graph: Graph,
    score: GraphScore,
    *,
    limit: int,
    ...
    """
    Degree-preserving 2-switches ranked by how many currently proved C8
    witnesses their removed edges hit.

    The full scorer remains authoritative: this only changes proposal order and
    concentration, not acceptance.
    """
    if limit <= 0:
        return []
    c8_count = _component_observed(score, 8)
    if c8_count <= 0:
        return []
    ...
    edge_set = set(graph.edges)
    graph_seed = int(graph.graph_hash[:16], 16)
    rng = random.Random(seed ^ graph_seed ^ 0xC8005A17)
    ranked: list[
        tuple[tuple[int, int, int, float], Mutation]
    ] = []

    for first, second in disjoint_edge_pairs(graph.edges):
        mask = edge_masks.get(first, 0) | edge_masks.get(second, 0)
        coverage = mask.bit_count()
        if coverage == 0:
            continue
        hot_edges = int(first in edge_masks) + int(second in edge_masks)
        a, b = first
        c, d = second
        remaining = edge_set - {first, second}
        pairings = (
        ...
        for added_edges, variant in pairings:
            ...
            ranked.append(
                (
                    (-coverage, -hot_edges, len(added_edges), rng.random()),
                    Mutation(
                        family="c8_two_switch",
                        removed_edges=(first, second),
                        added_edges=added_edges,
                        variant=f"{variant}/c8cover={coverage}/hot={hot_edges}",
                    ),
                )
            )

    ranked.sort(key=lambda item: item[0])
    return [mutation for _, mutation in ranked[:limit]]


def c8_endpoint_relocate_mutations(
    graph: Graph,
    score: GraphScore,
    *,
    limit: int,
    ...
    """
    Endpoint relocations restricted to edges lying on current C8 witnesses.

    Only a donor with degree>=4 is legal. This family therefore becomes useful
    specifically after a slack edge has created non-cubic degree freedom.
    """
    if limit <= 0:
        return []
    c8_count = _component_observed(score, 8)
    ...
    for cycle in cycles:
        edge_load.update(cycle)

    degrees = vertex_degrees(graph)
    edge_set = set(graph.edges)
    graph_seed = int(graph.graph_hash[:16], 16)
    rng = random.Random(seed ^ graph_seed ^ 0xC8E0A7E)
    ranked: list[tuple[tuple[int, int, float], Mutation]] = []
    ...
    for edge, load in edge_load.items():
        ...
        for fixed, donor in ((a, b), (b, a)):
            ...
            for target in range(graph.order):
                ...
                ranked.append(
                    (
                        (-load, -degrees[donor], rng.random()),
                        Mutation(
                            family="c8_endpoint_relocate",
                            removed_edges=(edge,),
                            added_edges=(added,),
                            variant=(
                                f"fixed={fixed}/donor={donor}/target={target}"
                                f"/c8load={load}"
                            ),
                        ),
                    )
...
def mutations_for_family(
    ...
    *,
    ...
    if family == "slack_edge_add":
        return slack_edge_add_mutations(
            graph,
            max_edge_excess=max_edge_excess,
        )
    if family == "c8_two_switch":
        return c8_two_switch_mutations(
            graph,
            score,
            limit=c8_limit,
            seed=seed,
        )
    if family == "c8_endpoint_relocate":
        return c8_endpoint_relocate_mutations(
            graph,
            score,
            limit=c8_limit,
            seed=seed,
...
def main() -> int:
    ...
    console.print(
        f"[bold]HEG two-phase C8 cleanup + exact C4 gate[/bold] "
        ...
        f"cleanup_families={','.join(args.cleanup_families)} "
        f"target_C4<={args.target_c4_cap} "
        f"cleanup_escape={args.cleanup_escape_height} "
        f"success_TOTAL<={args.success_total} "
        f"c8_limit={args.c8_limit_per_parent}"
    )
    console.print(
    ...
    with ProcessPoolExecutor(
        ...
        root_c4 = _component_observed(initial_score, 4)
        cleanup_active = root_c4 <= args.target_c4_cap
        cleanup_best_total: int | None = (
            initial_score.total if cleanup_active else None
        )
        cleanup_frontier: BeamState | None = root if cleanup_active else None
        ...
        for depth in range(1, args.max_depth + 1):
            ...
            active_families = (
                args.cleanup_families if cleanup_active else args.families
            )
            buckets = build_buckets(
                ...
                c8_limit_per_parent=args.c8_limit_per_parent,
            ...
            c4_ceiling = (
                args.target_c4_cap
                if cleanup_active
                else max(root_c4, args.target_c4_cap)
            )
            ...
            constrained_pool = [
                state
                for state in exact_pool
                if _component_observed(state.score, 4) <= args.target_c4_cap
            ]
            if not cleanup_active and constrained_pool:
                cleanup_active = True
                cleanup_best_total = min(
                    state.score.total
                    for state in constrained_pool
                    if state.score.total is not None
                )
                console.print(
                    f"[bold cyan]ENTER CLEANUP PHASE[/bold cyan] depth={depth} "
                    f"C4<={args.target_c4_cap} cleanup_best_TOTAL="
                    f"{cleanup_best_total}; subsequent scoring enforces the "
                    "C4 cap exactly."
                )

            # Scientific success is TOTAL, irrespective of component profile.
            success_hits = [
                state
                for state in exact_pool
                if state.score.total is not None
                and state.score.total <= args.success_total
            ]
            if success_hits:
                hit_state = min(
                    success_hits,
                    key=lambda state: (
                        state.score.total,
                        _component_observed(state.score, 8),
                ...
                console.print(
                    f"[bold red]TARGET REACHED[/bold red] "
                    f"TOTAL={hit_state.score.total} "
                    f"{components_text(hit_state.score)} depth={depth} "
                    f"saved: {path}"
                )
                ...
                if args.save_frontier is not None:
                    save_graph(args.save_frontier, hit_state)
                    augment_saved_graph_with_structural(
                        args.save_frontier,
                        state=hit_state,
                        metrics=hit_metrics,
            ...
            if cleanup_active:
                selection_pool = constrained_pool
                if not selection_pool:
                    console.print(
                        f"[yellow]Depth {depth}: cleanup phase produced no "
                        f"exact state with C4<={args.target_c4_cap}.[/yellow]"
                    )
                    break
                depth_cleanup_best = min(
                    state.score.total
                    for state in selection_pool
                    if state.score.total is not None
                )
                cleanup_best_total = (
                    depth_cleanup_best
                    if cleanup_best_total is None
                    else min(cleanup_best_total, depth_cleanup_best)
                )
                selection_best_total = cleanup_best_total
                selection_escape_height = args.cleanup_escape_height
            else:
                selection_pool = exact_pool
                selection_best_total = global_best_total
                selection_escape_height = args.escape_height

            barrier = selection_best_total + selection_escape_height

            shortlist, shortlist_best_count, shortlist_escape_count = (
                stratified_structural_shortlist(
                    selection_pool,
                    global_best_total=selection_best_total,
                    escape_height=selection_escape_height,
            ...
            if cleanup_active:
                frontier_candidates = [
                    state
                    for state in analyzed
                    if state.score.total == cleanup_best_total
                ]
                if (
                    cleanup_frontier is not None
                    and cleanup_frontier.score.total == cleanup_best_total
                ):
                    if cleanup_frontier.score.graph.graph_hash not in structural_cache:
                        structural_cache[cleanup_frontier.score.graph.graph_hash] = (
                            compute_structural_metrics(
                                cleanup_frontier,
                                max_length=args.max_structural_cycle_length,
                            )
                        )
                    frontier_candidates.append(cleanup_frontier)
                if frontier_candidates:
                    cleanup_frontier = min(
                        frontier_candidates,
                        key=lambda state: structural_rank_key(
                            state,
            ...
            console.print(
                ...
                f"phase={'cleanup' if cleanup_active else 'acquire'} "
                f"phase_best={selection_best_total} "
                f"barrier<={barrier} "
            ...
            depth_stats = family_depth_stats(exact_pool, active_families)
            for family in active_families:
    ...
    if args.save_frontier is not None and cleanup_frontier is not None:
        save_graph(args.save_frontier, cleanup_frontier)
        frontier_metrics = structural_cache.get(
            cleanup_frontier.score.graph.graph_hash
        )
        augment_saved_graph_with_structural(
            args.save_frontier,
            state=cleanup_frontier,
            metrics=frontier_metrics,
        )
        console.print(
            f"Cleanup frontier saved: {args.save_frontier} "
            f"TOTAL={cleanup_frontier.score.total} "
            f"{components_text(cleanup_frontier.score)}"
        )
    ...
    if cleanup_frontier is not None:
        console.print(
            f"[bold]Cleanup frontier[/bold] "
            f"TOTAL={cleanup_frontier.score.total} "
            f"{components_text(cleanup_frontier.score)} "
            f"m={len(cleanup_frontier.score.graph.edges)} "
            f"hash={cleanup_frontier.score.graph.graph_hash[:8]}"
        )
