        ...
        *,
        ...
        if not self._printed_header:
            ...
            console.print(
                ...
                f"{cycle_headers} {'ΔSUM':>10} {'TIME':>8}  STATE"
            )
        ...
        for length in self.lengths:
            ...
            if component is None:
                cycle_cells.append(f"{'-':>19}")
                continue

            lower, upper = delta_interval(leader, length)
            if delta_exact(leader, length):
                delta_text = f"+{lower}"
            else:
                delta_text = f"[+{lower},+{upper}]"

            absolute = compact_value(component)
            cycle_cells.append(f"{(delta_text + ' [' + absolute + ']'):>19}")
        ...
        console.print(
            ...
            + " ".join(cycle_cells)
            + f" {delta_sum:>10} {elapsed:>7.2f}s  {score_status(leader.score)}"
        )
