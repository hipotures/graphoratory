#!/usr/bin/env python3
"""
HEG fixed-order coupled-switch experiment.

Starting point
--------------
Current best n=26 graph:

    C4=7, C8=0, C16=0, TOTAL=7
    tauE=7, tauV=4
    maxEdgeLoad=1, maxVertexLoad=3

Previous experiments established:

- ordinary two_switch does not cross 7 -> 6 in the searched plateau;
- endpoint_relocate found TOTAL=7 but did not cross 7 -> 6;
- guided vertex_rewire2 did not cross 7 -> 6 directly;
- vertex_rewire2 + escape_height=1 can perform 8 -> 7, but still did not find 6;
- the old vertex_rewire3 generator had no legal proposals because it required
  three donor endpoints to remain degree >=3 after independent relocations.

New operator: coupled_switch2
-----------------------------
Choose a hot vertex v and two forbidden-cycle edges

    (v,a), (v,b)

preferably covering two different forbidden cycles.

Choose two additional existing partner edges

    (x,c), (y,d)

and atomically perform TWO 2-switches:

    remove:
        (v,a), (v,b), (x,c), (y,d)

    add one legal reconnection for each pair:
        (v,x),(a,c)  OR  (v,c),(a,x)
        (v,y),(b,d)  OR  (v,d),(b,y)

The two partner edges may also be forbidden-cycle edges. Candidate plans are
ranked by the number of DISTINCT forbidden cycles hit by all four removed
edges. Thus one atom can deliberately attack 2, 3, or 4 forbidden cycles.

All four removed edges and all four added edges are distinct. The hot edges,
their donor endpoints, and the two partner edges are chosen vertex-disjoint
enough to make the operation a clean degree-preserving 4-edge rewrite.

Properties:

    Δorder = 0
    Δedges = 0
    full degree sequence preserved

This avoids the degree-3 obstruction that made the old vertex_rewire3 empty.

Why this is genuinely new
-------------------------
The final graph of a coupled_switch2 can be reachable by two ordinary
2-switches, but the intermediate graph may have TOTAL well above the beam
barrier. Evaluating the composition atomically therefore explores states that
the sequential search can systematically prune.

Performance
-----------
This script keeps the optimized scoring pipeline:

- candidate mutation descriptors generated in the parent;
- fixed-order apply/validation/hash done in score processes;
- spawn-based ProcessPoolExecutor;
- one persistent sglab ScoreWorker subprocess per score process;
- score IPC in batches.

Typical prior throughput was ~70k candidates in ~7-8 s/depth.

Recommended first run
---------------------
Direct/neutral search only: test whether the new atom can break TOTAL=7 without
an escape band.

    uv run python scripts/heg_fixed_order_coupled_switch.py \
      --start-graph surgery_escape1_best_n26.json \
      --beam-width 32 \
      --main-lanes 24 \
      --structural-lanes 8 \
      --escape-lanes 0 \
      ...
      --families two_switch,endpoint_relocate,coupled_switch2 \
      --guided-limit-per-parent 1024 \
      --hot-vertices-per-parent 6 \
      --node-budget 10000000 \
      --witness-cap 1000000 \
      --save-best coupled_best_n26.json \
      --save-lineage coupled_lineage_n26.json
"""
...
def guided_coupled_switch2_mutations(
    ...
    *,
    ...
    """
    Generate atomic pairs of 2-switches anchored at one hot forbidden vertex.

    We remove two forbidden edges incident to `center`, plus two partner edges.
    All degrees are preserved exactly.

    Ranking:
      1. maximize distinct forbidden cycles hit by the four removed edges;
      2. maximize distinct cycles hit by the two hot edges;
      3. maximize hot vertex load;
      4. prefer partner edges with higher forbidden-cycle coverage;
      5. deterministic seeded diversity.

    The generator returns at most `limit` candidates per beam parent.
    """
    if limit <= 0:
        return []
    ...
    graph_hash_seed = int(graph.graph_hash[:16], 16)
    rng = random.Random(seed ^ graph_hash_seed)

    for center in hot_vertices:
        hot_edges = sorted(
            edge
            for edge, cycle_mask in edge_masks.items()
            if cycle_mask != 0 and center in edge
        )
        if len(hot_edges) < 2:
            continue
        ...
        for hot_1, hot_2 in combinations(hot_edges, 2):
            ...
            if hot_coverage < 2:
                # We specifically want one atom to attack multiple distinct
                # forbidden cycles around the same hot vertex.
                continue

            _, donor_1 = _center_oriented_edge(hot_1, center)
            _, donor_2 = _center_oriented_edge(hot_2, center)
            blocked_vertices = {center, donor_1, donor_2}

            partner_edges = [
                edge
                for edge in graph.edges
                if edge not in (hot_1, hot_2)
                and blocked_vertices.isdisjoint(edge)
            ]
            if len(partner_edges) < 2:
                continue

            # Prefer partner-edge pairs that are themselves useful forbidden
            # edges, but keep all legal possibilities available.
            partner_edges.sort(
                key=lambda edge: (
                    -edge_masks.get(edge, 0).bit_count(),
                    edge,
                )
            )

            # For n=26 the partner-edge pool is small enough to enumerate clean
            # vertex-disjoint pairs. The final global cap still bounds scoring.
            for partner_1, partner_2 in combinations(partner_edges, 2):
                ...
                total_coverage = removal_mask.bit_count()
                partner_coverage = (
                    edge_masks.get(partner_1, 0)
                    | edge_masks.get(partner_2, 0)
                ).bit_count()

                # Seeded tie breaker avoids repeatedly presenting identical
                # lexicographic neighborhoods across neutral plateau states.
    ...
    # We deliberately overscan plans because some 4-edge reconnections are
    # illegal due to pre-existing added edges. Stop once the requested number of
    # VALID mutation descriptors has been assembled.
    for (
        rank,
        center,
        hot_1,
        ...
        # Re-orient hot edges semantically as (center, donor). This matters
        # because Edge normalization alone would otherwise lose which endpoint
        # should remain the common hot vertex.
        _, donor_1 = _center_oriented_edge(hot_1, center)
        _, donor_2 = _center_oriented_edge(hot_2, center)
        oriented_hot_1 = (center, donor_1)
        oriented_hot_2 = (center, donor_2)

        # Both assignments of partner edges to the two hot edges are distinct
        # atomic moves and can yield different final graphs.
        assignments = (
        ...
        total_coverage = -rank[0]
        hot_coverage = -rank[1]
        partner_coverage = -rank[3]
        ...
        for first_partner, second_partner, assignment_name in assignments:
            ...
            for first_variant, first_added in enumerate(first_options):
                for second_variant, second_added in enumerate(second_options):
                    ...
                    # Require a genuine 4-edge rewrite.
                    if len(set(added)) != 4:
                        continue
...
def main() -> int:
    ...
    console.print(
        f"[bold]HEG fixed-order coupled switch[/bold] "
