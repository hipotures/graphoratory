...
def test_hash_and_typed_display_identifiers_are_lowercase() -> None:
    ...
    assert all(len(identifier.hash_full) == 64 for identifier in identifiers.values())
    ...
    assert identifiers[ObjectType.CORPUS].display.startswith("cp-")
    assert all(
...
def test_short_and_full_typed_identifier_resolution() -> None:
    ...
    assert resolve_typed("ws-a1b2c3d4", ObjectType.WORKSPACE, [full_hash]).hash_full == full_hash
    assert (
        resolve_typed(f"ws-{full_hash}", ObjectType.WORKSPACE, [full_hash]).hash_full == full_hash
    )
